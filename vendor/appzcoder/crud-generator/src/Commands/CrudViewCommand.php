...
class CrudViewCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $this->crudLabel = strtolower($this->option('label'));
        $this->varName = lcfirst($this->argument('name'));
        $this->varLabel = lcfirst($this->option('label'));
        $this->crudNameCap = ucwords($this->crudName);
        $this->crudLabelCap = ucwords($this->crudLabel);
        $this->crudNameSingular = str_singular($this->crudName);
        $this->crudLabelSingular = str_singular($this->crudLabel);
        ...
        foreach ($this->formFields as $key => $value) {
            /*
            ThuanDH: Không cần giới hạn số colunm hiển thị ra ngoài
            ...
            */
            ...
            $this->formHeadingHtml .= '<th>' . $labelDisplay . '</th>';
            $this->formBodyHtml .= '<td>{{ $item->' . $field . ' }}</td>';
            $this->formBodyHtmlForShowView .= '<tr><th> ' . $labelDisplay . ' </th><td> {{ $%%crudNameSingular%%->' . $field . ' }} </td></tr>';
    ...
    {
        return [
            'index' => ['formHeadingHtml', 'formBodyHtml', 'crudLabel', 'crudLabelCap', 'crudLabelSingular','crudName' , 'crudNameCap', 'modelName', 'viewName', 'routeGroup', 'primaryKey'],
            'form' => ['formFieldsHtml'],
            'create' => ['crudLabel', 'crudLabelCap', 'crudLabelSingular', 'crudName', 'crudNameCap', 'modelName', 'modelNameCap', 'viewName', 'routeGroup', 'viewTemplateDir'],
            'edit' => ['crudLabel', 'crudLabelCap', 'crudLabelSingular', 'crudName', 'crudNameSingular', 'crudNameCap', 'modelNameCap', 'modelName', 'viewName', 'routeGroup', 'primaryKey', 'viewTemplateDir'],
            'show' => ['formHeadingHtml', 'formBodyHtml', 'formBodyHtmlForShowView', 'crudLabel', 'crudLabelCap', 'crudLabelSingular', 'crudName', 'crudNameSingular', 'crudNameCap', 'modelName', 'viewName', 'routeGroup', 'primaryKey'],
        ];
    }
    ...
    protected function wrapField($item, $field)
    {
        ...
        $labelText = "'" . ucwords(strtolower(str_replace('_', ' ', $item['label']))) . "'";

        if ($this->option('localize') == 'yes') {
            $labelText = 'trans(\'' . $this->crudName . '.' . $item['name'] . '\')';
        }

        return sprintf($formGroup, $item['name'], $labelText, $field, $required);
    }
